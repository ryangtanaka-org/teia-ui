...
//
// This needs to be overhauled once an indexer is in place.
...
function normalizeToken(token: any): PickerToken {
  return {
    ...
    preview_uri: token.teia_meta?.preview_uri || token.preview_uri || undefined,
    artist_address: token.artist_address || undefined,
  }
}

/** Thumbnail for a token tile: imgproxy preview when present, else CDN. */
